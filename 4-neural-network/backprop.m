...
% total_error - total squared error of the ANN
  ...
  for i=1:rows(input)
    ...
    delta_out = (output_from_output - desired_output) .* output_from_output .* (1 - output_from_output);

    delta_out_w = output_from_hidden' * delta_out;
    delta_out_b = delta_out;

    % 7. Compute delta error of the hidden layer
    % how many delta errors should be computed here?
    delta_hid = delta_out * output_layer(1:end-1,:)' .* output_from_hidden .* (1 - output_from_hidden);

    delta_hid_w = input(i,:)' * delta_hid;
    delta_hid_b = delta_hid;

    % 8. Update output layer weights
    output_layer -= lr * [delta_out_w; delta_out_b];

    % 9. Update hidden layer weights
    hidden_layer -= lr * [delta_hid_w; delta_hid_b];
  end
